...
class BiFPN(layers.Layer):
    def __init__(self, in_channels):
        ...
        self.out_convs.append(tf.keras.Sequential([layers.SeparableConv2D(in_ch[0], 3, padding='same'),
                                                   layers.Activation('relu'),
                                                   layers.BatchNormalization()]))
        for i in range(self.input_layer_cnt-2):
            self.td_weights.append(tf.random.normal([2]))
            self.td_convs.append(tf.keras.Sequential([layers.SeparableConv2D(in_ch[i+1], 3, padding='same'),
                                                   layers.Activation('relu'),
                                                      layers.BatchNormalization()]))
            self.out_weights.append(tf.random.normal([3]))
            self.out_convs.append(tf.keras.Sequential([layers.SeparableConv2D(in_ch[i+1], 3, padding='same'),
                                                   layers.Activation('relu'),
                                                       layers.BatchNormalization()]))
        self.td_weights.append(tf.random.normal([2]))
        self.td_convs.append(tf.keras.Sequential([layers.SeparableConv2D(in_ch[-1], 3, padding='same'),
                                                   layers.Activation('relu'),
                                                   layers.BatchNormalization()]))

        self.upconvs  = [tf.keras.Sequential([layers.UpSampling2D(u),
                                              layers.SeparableConv2D(c,k,padding=pad)])
...
def CustomNet(input_dim, num_stages, num_blocks, num_classes, group_width, name='None'):
    ...
    output_list = [(output.shape.as_list()[1], output.shape.as_list()[3]) for output in xs]
    print(output_list[::-1])
    print(xs[::-1])
    x = BiFPN(output_list[::-1])(xs[::-1])
    #x = BiFPN(output_list[::-1])(x[::-1])
    bifpn_output = BiFPN(output_list[::-1])(x[::-1])
    ...
    for bifpn_output in x[1:]:
        print(bifpn_output)
        # detection
        x = layers.Conv2D(4, 1, strides=1, use_bias=False)(bifpn_output)
